...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT user.id,tai_khoan,ho_ten,gioi_tinh,ngay_sinh,so_dien_thoai,email,anh_dai_dien,dia_chi,cmt,user.status,phong_ban.ten_phong_ban,chuc_vu.ten_chuc_vu,hop_dong.tinh_chat_lao_dong,hop_dong.id_loai_hop_dong,user.ngay_tao,user.ngay_sua\n" +
            "FROM user \n" +
            "JOIN phong_ban ON phong_ban.id = user.id_phong_ban\n" +
            "JOIN chuc_vu ON chuc_vu.id = user.id_chuc_vu\n" +
    ...
    @Query(nativeQuery = true, value = "SELECT user.id,tai_khoan,ho_ten,gioi_tinh,ngay_sinh,so_dien_thoai,email,anh_dai_dien,dia_chi,cmt,user.status,phong_ban.ten_phong_ban,chuc_vu.ten_chuc_vu,hop_dong.tinh_chat_lao_dong,hop_dong.id_loai_hop_dong,user.ngay_tao,user.ngay_sua FROM user \n" +
            "JOIN phong_ban ON phong_ban.id = user.id_phong_ban\n" +
            "JOIN chuc_vu ON chuc_vu.id = user.id_chuc_vu\n" +
            "JOIN hop_dong ON hop_dong.id_user = user.id\n" +
            "WHERE hop_dong.status = 1 \n" +
